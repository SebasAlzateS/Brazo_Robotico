...
class serialArduino:
    def __init__(self,port,baud=9600,sizeData=2):
        self.port = port
        self.baud = baud
    ...
    def backgroundThread(self):
        ...
        while(self.isRun):
            if(self.serialConnection.readline().strip().decode("uft-8") == "E"):
        ...
        for k in range(sizeSendData):
            if k < sizeSendData-1:
               stringData = stringData + str(dataToSend[k]) + ','
                ...
                stringData = stringData + str(dataToSend[k])
        self.serialConnection.write((stringData + '-').encode()) #cambiar por menos para arduino
    def close(self):
        self.isRun = False
        if self.thread == None:
            pass
